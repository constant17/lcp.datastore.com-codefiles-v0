...
<?php
	...
	else if(isset($_POST['search_doc']))
	{
		...
		$print_result = $get_connected->get_result_by_type(str_replace("-", " ",$_POST['search_doc']), 'doc');
		$key_definition = $get_connected->define_the_key(trim(str_replace("-", " ",$_POST['search_doc'])), $path);
		if($key_definition != "")
			$key_definition = "<div class='dfinition'><div class='wordwrap'><u>D&eacute;finition de la cl&eacute; de recherche</u><br>".$key_definition."</div></div>";
		else $key_definition = "";
